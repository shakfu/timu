...
def _env(ctx: Context) -> dict[str, str]:
    ...
    HOME is the private temp dir, so tools find no user config."""
    ...
    if not any(ctx.workdir.is_relative_to(r) for r in ctx.write_roots):
        # Read-only workspace (reviewer): keep Python caches out of it (design 4.3).
        env["PYTHONPYCACHEPREFIX"] = str(ctx.tmp / "pycache")
...
def _shell(ctx: Context, args: Mapping[str, Any]) -> ToolOutput:
    ...
    try:
        ...
        return ToolOutput(f"cannot run {argv[0]}: {e.strerror}", is_error=True)
    assert proc.stdout is not None
    out, fd = _Output(ctx.max_output), proc.stdout.fileno()
    ...
    if stopped:
        return ToolOutput(f"{text}{nl}[{stopped}; killed]", is_error=True)
    code = proc.returncode if proc.returncode >= 0 else 128 - proc.returncode
    return ToolOutput(f"{text}{nl}[exit {code}]", is_error=code != 0)
